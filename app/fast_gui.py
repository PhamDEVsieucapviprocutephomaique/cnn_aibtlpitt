"""
FAST GUI - Sử dụng fast inference engine
"""
import os
import sys
import numpy as np
...
from src.fast_inference import get_fast_engine


class FastDrawingApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("⚡ Fast Neural Recognition")
        self.root.geometry("800x600")
        
        # Load fast engine
        try:
            self.engine = get_fast_engine()
            self.engine_status = "✅ Fast Engine Ready!"
            print("Fast models loaded!")
    ...
    def setup_ui(self):
        ...
        # Title
        title = ttk.Label(main_frame, text="⚡ Fast Neural Recognition", 
                         font=('Arial', 16, 'bold'))
        ...
        # Model info
        info_text = ("⚡ Fast Models:\n"
                    "• MNIST: ~99% (28x28)\n"
                    "• Shapes: ~95% (48x48)\n"
                    "• Trained in ~45 seconds\n"
                    "• Inference: <50ms")
    ...
    def predict(self):
        ...
        try:
            ...
            self.type_label.config(text=f"{result['type']} ({elapsed:.0f}ms)")
            ...
            print(f"Prediction: {result['result']} ({result['confidence']:.1%}) in {elapsed:.1f}ms")
...
def main():
    try:
        print("Starting Fast Neural Recognition App...")
        app = FastDrawingApp()
        app.root.mainloop()
    except Exception as e:
